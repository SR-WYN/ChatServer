...
{
    _fun_callbacks[MSG_CHAT_LOGIN] = [](std::shared_ptr<CSession> session, const short &msg_id,
                                        const std::string &msg_data) {
        LoginHandler::handleLogin(session, msg_id, msg_data);
    };
    _fun_callbacks[MSG_SEARCH_USER_REQ] = [](std::shared_ptr<CSession> session, const short &msg_id,
                                             const std::string &msg_data) {
        UserHandler::handleSearchUser(session, msg_id, msg_data);
    };
    _fun_callbacks[MSG_ADD_FRIEND_REQ] = [](std::shared_ptr<CSession> session, const short &msg_id,
                                            const std::string &msg_data) {
        FriendHandler::handleAddFriend(session, msg_id, msg_data);
    };
    _fun_callbacks[MSG_AUTH_FRIEND_REQ] = [](std::shared_ptr<CSession> session, const short &msg_id,
                                             const std::string &msg_data) {
        FriendHandler::handleAuthFriend(session, msg_id, msg_data);
    };
    _fun_callbacks[MSG_TEXT_CHAT_MSG_REQ] = [](std::shared_ptr<CSession> session,
                                               const short &msg_id, const std::string &msg_data) {
        ChatMessageHandler::handleTextChat(session, msg_id, msg_data);
    };
    _fun_callbacks[MSG_CHAT_HISTORY_REQ] = [](std::shared_ptr<CSession> session,
                                              const short &msg_id, const std::string &msg_data) {
        ChatQueryHandler::handleHistory(session, msg_id, msg_data);
    };
    _fun_callbacks[MSG_HEARTBEAT_PING] = [](std::shared_ptr<CSession> session, const short &msg_id,
                                            const std::string &msg_data) {
