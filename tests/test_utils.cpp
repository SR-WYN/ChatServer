// test_utils.cpp - ChatServer 纯函数工具类单元测试
...
class SteadyMsTest : public ::testing::Test
{
};

TEST_F(SteadyMsTest, ReturnsNonZero)
{
    auto ms = utils::time::steadyMs();
    EXPECT_GT(ms, 0u);
}

TEST_F(SteadyMsTest, MonotonicallyIncreasing)
{
    auto t1 = utils::time::steadyMs();
    auto t2 = utils::time::steadyMs();
    EXPECT_GE(t2, t1);
}
